    ...
    const fetchCommunities = async () => {
      try {
        const [joined, created] = await Promise.all([
          fetch(
            `http://localhost:8000/api/user-joined-community/?id=${localStorage.getItem("UserId")}`,
            {
              method: "GET",
              headers: {
          ...
          fetch(
            `http://localhost:8000/api/user-created-community/?id=${localStorage.getItem("UserId")}`,
            {
              method: "GET",
              headers: {
                Authorization: `Bearer ${localStorage.getItem("access_token")}`,
        ...
        if (!created_community && !joined_community) uniqueCommunities = [];
        else if (!created_community)
          uniqueCommunities = [
            ...new Map([...joined_community].map((c) => [c.id, c])).values(),
          ];
        else if (!joined_community)
          uniqueCommunities = [
          ...
          uniqueCommunities = [
            ...new Map(
              [...created_community, ...joined_community].map((c) => [c.id, c])
            ).values(),
          ];
