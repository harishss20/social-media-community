    ...
    <div className="flex flex-grow justify-center items-center h-screen bg-[#1a1a1a]">
      <div className="w-[90%] max-w-lg bg-[#343538] p-6 rounded-xl shadow-md">
        <div className="flex justify-between items-center mb-4">
          ...
          <button className="py-2 px-4 duration-300">continue</button>
        </div>

        <div className=" mt-12 space-y-6">
          {data.map((communities) => (
            <CommunityCard key={communities.id} communities={communities} />
          ))}
        </div>
      </div>
    </div>
  ...
  return (
    <div className="flex items-center justify-between  bg-[#2b2c2e] px-4 py-3 rounded-lg">
      <div className="flex items-center space-x-4">
        ...
        />
        <p className="text-white text-lg ">{communities.name}</p>
      </div>
      <button className="px-4 py-2 bg-black text-white rounded-lg duration-300">
        Join
      </button>
    </div>
  );
